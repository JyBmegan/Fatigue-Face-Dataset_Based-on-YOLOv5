...
# one trait: 眨眼
    ...
    blink_counter = 0
    blink_durations = []  # 存储每次眨眼的持续帧数
    blink_timestamps = []  # 存储每次眨眼发生的时间点（秒）
    is_blinking = False
    ...
    while cap.isOpened():
        ...
        if len(rects) > 0:
            ...
            ear = (calculate_ear(shape[lStart:lEnd]) + calculate_ear(shape[rStart:rEnd])) / 2.0
            ...
            if ear < EAR_THRESHOLD:
                ...
                if consecutive_frames_counter >= EAR_CONSEC_FRAMES:
                    blink_counter += 1
                    blink_durations.append(consecutive_frames_counter)
                    blink_timestamps.append(current_frame_pos / fps)
                consecutive_frames_counter = 0
                is_blinking = False
    ...
    if total_frames_processed == 0:
        return {'video_id': os.path.basename(video_path).split('.')[0]}

    duration_sec = total_frames_processed / fps

    # 特征1: 眨眼频率 (Blink Frequency)
    ...
    # 特征3: 平均眨眼持续时间 (Avg Blink Duration) in seconds
    avg_blink_duration = (np.mean(blink_durations) / fps) if blink_durations else 0

    # 特征4: 最长闭眼时间 (Max Closure Duration) in seconds
    max_closure_duration = (np.max(blink_durations) / fps) if blink_durations else 0

    # 特征5: 眨眼频率的标准差 (Blink Rate Variability)
    inter_blink_intervals = np.diff(blink_timestamps) if len(blink_timestamps) > 1 else []
    blink_rate_variability = np.std(inter_blink_intervals) if len(inter_blink_intervals) > 0 else 0
    ...
    return {
        ...
        'blink_rate_variability': blink_rate_variability
    }


def main():

    print("--- 任务开始：疲劳检测模型训练 (增强特征版) ---")
    ...
    if not pre_test_files or not post_test_files:
        print(f"\n错误：在文件夹 '{label_folder}' 中找不到 pretest 或 posttest 的xlsx文件。")
        sys.exit()

    pre_df = pd.read_excel(pre_test_files[0])
    post_df = pd.read_excel(post_test_files[0])
    ...
    all_features = []
    print(f"\n[进度] 开始从 {len(video_paths)} 个视频中提取增强特征...")
    for video_path in sorted(video_paths):
        features = process_video(video_path, detector, predictor)
        if features:
            all_features.append(features)
    print("  > 所有视频的增强特征提取完毕！")
    features_df = pd.DataFrame(all_features).fillna(0)  # 使用fillna(0)确保没有NaN值

    print("\n[进度] 正在合并特征和标签...")
    final_df = pd.merge(features_df, labels_df, on='video_id', how='inner')
    final_df.to_csv('fatigue_features_enhanced.csv', index=False)
    print(f"  > 合并完成！共找到 {len(final_df)} 个匹配数据，已保存至 fatigue_features_enhanced.csv")

    print("\n[进度] 检查标签类别分布...")
    print(final_df['疲劳等级'].value_counts())

    print("\n[进度] 开始训练分类模型...")
    if len(final_df) < 5:
        print("  > 错误：匹配到的数据不足5条，无法训练模型。")
        sys.exit()


    feature_columns = ['blink_freq', 'perclos', 'avg_blink_duration', 'max_closure_duration', 'blink_rate_variability']
    ...
    print("  > 正在训练随机森林模型 (使用新特征)...")
    model = RandomForestClassifier(n_estimators=100, random_state=42, class_weight='balanced')  # 添加class_weight来处理不平衡问题
    model.fit(X_train, y_train)
    print("  > 模型训练完毕！")

    joblib.dump(model, 'fatigue_model_enhanced.joblib')
    print("  > 增强版模型已保存至 fatigue_model_enhanced.joblib")

    print("\n[进度] 开始评估增强版模型并生成可视化结果...")
    y_pred = model.predict(X_test)
    print(f"\n  > [新]模型在测试集上的准确率: {accuracy_score(y_test, y_pred):.2%}")
    print("\n  > [新]分类报告:")
    ...
    print("\n  > 增强版混淆矩阵图已保存至 confusion_matrix_enhanced.png")
    ...
    print("  > 增强版特征重要性图已保存至 feature_importance_enhanced.png")
